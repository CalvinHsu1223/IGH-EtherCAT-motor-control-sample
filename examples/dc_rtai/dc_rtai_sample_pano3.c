...
//    while (deactive!=20) {
    while (1) {
		...
		inpu[3]=EC_READ_U32(domain1_pd + pos_act2);
                inpu[4]=EC_READ_U16(domain1_pd + status_word3);
                inpu[5]=EC_READ_U32(domain1_pd + pos_act3);
...
//		if(servooff==1){//servo off
	...
	//	}	
		...
		if( (inpu[0]&0x004f) == 0x0040  &&  (inpu[2]&0x004f) == 0x0040 && (inpu[4]&0x004f) == 0x0040){
		EC_WRITE_U16(domain1_pd+ctrl_word, 0x0006 );
		EC_WRITE_U16(domain1_pd+ctrl_word2, 0x0006 );
                EC_WRITE_U16(domain1_pd+ctrl_word3, 0x0006 );
		}
		else if( (inpu[0]&0x006f) == 0x0021 && (inpu[2]&0x006f) == 0x0021 && (inpu[4]&0x006f) == 0x0021){
		...
		else if( (inpu[0]&0x006f) == 0x0023 && (inpu[2]&0x006f) == 0x0023 && (inpu[4]&0x006f) == 0x0023){
		...
		else if( (inpu[0]&0x006f) == 0x0027  && (inpu[2]&0x006f) == 0x0027 && (inpu[4]&0x006f) == 0x0027){
...
//	ecrt_slave_config_dc(sc2, 0x300, 1000000, 0, 0, 0);
        ...
        if (!(sc3 = ecrt_master_slave_config(master, yas3, yaskawa))) {
            printk(KERN_ERR PFX "Failed to get slave configuration.\n");
            goto out_release_master;
        }
//      ecrt_slave_config_dc(sc3, 0x300, 1000000, 0, 0, 0);
