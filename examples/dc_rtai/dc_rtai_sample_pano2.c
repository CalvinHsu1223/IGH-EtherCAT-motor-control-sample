...
static unsigned int mode, mode2 ;
static unsigned int tar_torq, tar_torq2    ;
static unsigned int max_torq, max_torq2    ;
static unsigned int tar_pos, tar_pos2    ;
static unsigned int max_speed, max_speed2  ;
static unsigned int touch_probe_func, touch_probe_func2 ;
static unsigned int tar_vel, tar_vel2 ;
static unsigned int error_code, error_code2  ;
static unsigned int status_word, status_word2;
static unsigned int mode_display, mode_display2 ;
static unsigned int pos_act, pos_act2;
static unsigned int vel_act, vel_act2;
static unsigned int torq_act, torq_act2;
static unsigned int touch_probe_status, touch_probe_status2;
static unsigned int touch_probe_pos, touch_probe_pos2;
static unsigned int digital_input, digital_input2;
...
//    while (deactive!=20) {
    while (1) {
		...
		inpu[2]=EC_READ_U16(domain1_pd + status_word2);
		inpu[3]=EC_READ_U32(domain1_pd + pos_act2);
...
//		if(servooff==1){//servo off
	...
	//	}	
		...
		if( (inpu[0]&0x004f) == 0x0040  &&  (inpu[2]&0x004f) == 0x0040 ){
		EC_WRITE_U16(domain1_pd+ctrl_word, 0x0006 );
		EC_WRITE_U16(domain1_pd+ctrl_word2, 0x0006 );
		}
		...
		else if( (inpu[0]&0x006f) == 0x0023 && (inpu[2]&0x006f) == 0x0023){
		...
		else if( (inpu[0]&0x006f) == 0x0027  && (inpu[2]&0x006f) == 0x0027 ){
...
//	ecrt_slave_config_dc(sc2, 0x300, 1000000, 0, 0, 0);
